...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("debug_43.log"),  # Save logs to a file
...
class Gem5DVFSEnv(gym.Env):
        ...
        if is_low:
            ...
            if action >= 2:
                ...
                if cluster == "big":
                    ...
                    print(
                        f"[INFO] {cluster.capitalize()} High Load Detected with Action {action}  >= 4 - Adjusting to Action {action_val} Freq:{high_freq}")
                if duration_at_freq and duration_at_freq > 180:  # Example: 180 seconds threshold
                    print(f"[INFO] {cluster.capitalize()} Prolonged High Frequency Detected - Gradual Adjustment Down")
    ...
    def check_load_state(self,current_state, big_action, little_action, big_freq, little_freq):
        ...
        if big_low_load and not self.prev_low_load_state["big"]:  # Transition detected
            ...
            big_freq = 1200480  # Step down first
            big_action = 5
        ...
        if not little_low_load and self.prev_low_load_state["little"]:
            ...
            little_freq = 1901140  # Final transition (Max Frequency for Little Cores)
            little_action = 0
